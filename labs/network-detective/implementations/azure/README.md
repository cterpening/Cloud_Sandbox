# Azure implementation: Network Detective

Uses the [network-workshop module](../../../../modules/network-workshop/)
...
Edit the ignored file with your assigned group and an approved region. Supply `ssh_public_key` from a public SSH key you control; never commit its private
key. Leave `fault = "healthy"` for the first deployment. The VMs have no public IPs.
Run Command requires working VM agents and explicit outbound NAT (included).
...
use Function ZIP publishing or a Function key. The probe runs inside the client VM; the script returns only allowlisted booleans.
