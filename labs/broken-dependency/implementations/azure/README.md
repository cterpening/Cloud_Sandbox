# Azure implementation: The Broken Storage Dependency
...
Run commands from the repository root and select `$lab = 'broken-dependency'`.
...
```powershell
pwsh scripts/Publish-Lab.ps1 -Lab broken-dependency -PackageOnly
```

To check infrastructure without credentials:

```powershell
terraform -chdir=labs/broken-dependency/implementations/azure/terraform init -backend=false
terraform -chdir=labs/broken-dependency/implementations/azure/terraform validate
terraform -chdir=labs/broken-dependency/implementations/azure/terraform test
```
