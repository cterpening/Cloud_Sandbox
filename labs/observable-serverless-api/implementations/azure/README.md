# Azure implementation: Observable Serverless API
...
```powershell
pwsh scripts/Publish-Lab.ps1 -Lab observable-serverless-api -PackageOnly
```

To check infrastructure without credentials:

```powershell
terraform -chdir=labs/observable-serverless-api/implementations/azure/terraform init -backend=false
terraform -chdir=labs/observable-serverless-api/implementations/azure/terraform validate
terraform -chdir=labs/observable-serverless-api/implementations/azure/terraform test
```
